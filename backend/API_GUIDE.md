...
The Solar feature integrates external solar/energy systems (e.g. Fronius inverters). HomeForge stores **only the link** to the vendor's raw-JSON API; the backend fetches that JSON server-side and returns a **provider-agnostic, normalized schema** so a single reusable power-flow UI works across vendors.

> **Why a backend proxy?** Vendor APIs (like Fronius) have no CORS, are usually HTTP-only on private LAN IPs, and apply rate limits. The browser cannot call them directly. Poll the normalized `overview` endpoint instead.

### Provider support

| Provider key | Vendor |
|--------------|--------|
| `fronius` | Fronius Solar API V1 |

Additional providers can be added backend-side without changing the API contract below.

---

### 10.1 List Solar Systems

| Method | Endpoint | Auth Required | Role Required |
|--------|----------|---------------|---------------|
| `GET` | `/solar/systems/` | ✅ Yes | Any authenticated |

Returns a paginated list of registered solar systems.
  ...
  "results": [
    {
      "id": 1,
      "name": "Rooftop PV",
      "base_url": "http://fronius-server:9999",
      "provider": "fronius",
      "enabled": true,
      "api_version": "1",
...
### 10.2 Register Solar System

| Method | Endpoint | Auth Required | Role Required |
|--------|----------|---------------|---------------|
| `POST` | `/solar/systems/` | ✅ Yes | `admin` or `owner` |

Registers a new system. The backend validates the link by attempting discovery before saving; an unreachable or disabled API is rejected.
...
**Success Response (201 Created):** the created system object (see 10.1).

**Validation (400 Bad Request):**
```json
{ "base_url": "Could not reach the system at the provided link." }
```

---

### 10.3 Retrieve / Update / Delete Solar System

| Method | Endpoint | Auth Required | Role Required |
|--------|----------|---------------|---------------|
| `GET` | `/solar/systems/{id}/` | ✅ Yes | Any authenticated |
| `PUT` / `PATCH` | `/solar/systems/{id}/` | ✅ Yes | `admin` or `owner` |
| `DELETE` | `/solar/systems/{id}/` | ✅ Yes | `admin` or `owner` |

`api_version`, `capabilities`, and `last_seen` are read-only (managed by the backend).

---

### 10.4 Get Normalized Overview (Power Flow)

| Method | Endpoint | Auth Required | Role Required |
|--------|----------|---------------|---------------|
| `GET` | `/solar/systems/{id}/overview/` | ✅ Yes | Any authenticated |

Returns the live, normalized power-flow snapshot. The backend caches the result for ~8 seconds to respect vendor rate limits, so clients may poll every 5–10 seconds safely.
...
**Field semantics (frontend contract):**

| Field | Unit | Meaning / sign convention |
|-------|------|---------------------------|
| `power.solarW` | W | PV production, always ≥ 0 |
| `power.gridW` | W | **+** = importing from grid, **−** = exporting |
| `power.loadW` | W | House consumption, normalized **positive** |
| `power.batteryW` | W | **+** = charging, **−** = discharging |
| `battery.socPct` | % | State of charge (0–100), `null` if no battery |
| `energy.*` | Wh | Cumulative energy; **any field may be `null`** (e.g. GEN24) |
| `ratios.*` | % | May be `null` when no meter is present |
| `mode` | — | `bidirectional` \| `meter` \| `produce-only` \| `ac-coupled` \| `unknown` |

Any unavailable datapoint is `null`. Drive animated flow arrows from `power.*`: PV→home, PV→grid, grid→home, battery↔home. Render the battery node only when `battery.present`.

**Offline / error state (still HTTP 200):**
```json
{
  "provider": "fronius",
  "online": false,
  "mode": "unknown",
  "timestamp": null,
  "power": { "solarW": null, "gridW": null, "loadW": null, "batteryW": null },
...
When the system is unreachable, disabled on the device, or disabled in HomeForge, the endpoint returns `online: false` with a human-readable `status.message` (HTTP 200) so the UI can render a dead state without treating it as a hard error.
...
| Export/Import types | ✅ | ✅ | ❌ |
| View solar systems & overview | ✅ | ✅ | ✅ |
| Register/Edit/Delete solar systems | ✅ | ✅ | ❌ |
...
```typescript
type SolarProvider = 'fronius';

type SolarMode =
  | 'bidirectional'   // meter + battery
  | 'meter'           // meter, no battery
  | 'produce-only'    // PV only, no meter
  | 'ac-coupled'
  | 'unknown';
...
interface SolarSystem {
  id: number;
  name: string;
  base_url: string;                       // e.g. http://fronius-server:9999
  provider: SolarProvider;
  enabled: boolean;
  api_version: string;                    // cached, read-only
  capabilities: SolarCapabilities;        // cached, read-only
  last_seen: string | null;               // ISO datetime, read-only
  created_at: string;
  updated_at: string;
}
...
interface SolarOverview {
  provider: SolarProvider;
  online: boolean;                        // false => render dead state
  mode: SolarMode;
  timestamp: string | null;               // ISO datetime
  power: {
    solarW: number | null;                // PV production, >= 0
    gridW: number | null;                 // + import, - export
    loadW: number | null;                 // house consumption, positive
    batteryW: number | null;              // + charging, - discharging
  };
  battery: {
    present: boolean;
    socPct: number | null;                // 0-100
    mode: string | null;
  ...
  status: { code: number | null; message: string };
}
```
...
| `GET` | `/solar/systems/` | List solar systems | ✅ | Any |
| `POST` | `/solar/systems/` | Register solar system | ✅ | Admin |
| `GET` | `/solar/systems/{id}/` | Get solar system | ✅ | Any |
| `PUT` | `/solar/systems/{id}/` | Update solar system | ✅ | Admin |
| `DELETE` | `/solar/systems/{id}/` | Delete solar system | ✅ | Admin |
| `GET` | `/solar/systems/{id}/overview/` | Normalized power flow | ✅ | Any |
