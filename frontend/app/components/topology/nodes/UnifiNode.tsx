...
// Icon mapping closer to Unifi style
...
const UnifiNode = ({ data, selected }: NodeProps) => {
  // Parsing standard props + our enriched ones
  const details = (data.details as any) || data;
  const label = data.label || details.label || details.name || 'Unknown';
  const ip = details.ip || details.ip_address; // enriched via fetchDevices usually (api returns ip_address)
  const roomName = details.roomName; // enriched in TopologyPage
  const type = (details.type || details.device_type || 'client').toLowerCase();
  // Status check - backend_api.md usually sends 'status'
  ...
  return (
    <div className="group relative flex flex-col items-center justify-center pointer-events-none-none">
      <Handle 
        type="target" 
      ...
      <div className={clsx(
        ...
        selected && "opacity-0 translate-y-2 pointer-events-none" // Hide name when detailed view pops up? Or keep it?
        // Actually UniFi usually keeps name. Let's keep it simple.
      )}>
        {label}
      </div>
