...
Containerul backend nu definește un \texttt{CMD}/\texttt{ENTRYPOINT} implicit; comanda de pornire e comentată în \texttt{docker-compose.yml}, iar orchestrarea reală a serviciilor e în scriptul \texttt{run.sh}, rulat manual în interiorul containerului (de exemplu prin \texttt{docker compose exec backend sh run.sh}, după ce \texttt{docker compose up} a construit imaginile). Scriptul folosește \texttt{POSIX sh} (nu \texttt{bash}) pentru compatibilitate cu imagini Alpine sau slim unde \texttt{/bin/sh} nu este Bash. Forma sa, condensată pentru claritate:
\begin{minted}{bash}
#!/usr/bin/env sh
set -eu
...
Scriptul \texttt{migrate.sh} (apelat din \texttt{run.sh}) împachetează acest flux: rulează \texttt{makemigrations} pentru toate aplicațiile, apoi \texttt{migrate} pe fiecare bază de date detectată din \texttt{settings.DATABASES}. Scriptul nu așteaptă explicit disponibilitatea PostgreSQL: se bazează pe \texttt{depends\_on: db} din \texttt{docker-compose.yml} pentru ordinea de pornire a containerelor, nu pe o verificare activă a conexiunii. Decizia de a apela \texttt{makemigrations} la fiecare boot poate părea agresivă, dar în practică nu produce nimic dacă schema este sincronizată cu modelele. În schimb, dacă un dezvoltator a uitat să comite un fișier de migrație, scriptul îl generează automat la prima rulare în Docker.
...
\begin{enumerate}
    ...
    \item aprobarea cu \emph{Approve}: verificarea că în \texttt{/dashboard/device-types} apare noul tip și că în notificările contului care a aprobat (\texttt{owner}) apare mesajul \emph{Device Type Approved};
...
\end{enumerate}
Rezultatul, cu dispozitivul \emph{online} și cardul actualizat în dashboard, este arătat în Figura~\ref{fig:test3_real_device}.
...
\begin{table}[htb]
    ...
    \begin{tabular}{|p{1cm}|p{3.5cm}|p{4.2cm}|p{4.5cm}|}
        ...
        2 & Propunere $\rightarrow$ Approve & Tipul apare în \emph{Device Types}, notificare la contul care a aprobat & Confirmat: notificarea ajunge în mai puțin de 1 s în \emph{notification-center} \\ \hline
