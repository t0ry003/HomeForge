...
Toate apelurile către backend sunt concentrate în fișierul \texttt{apiClient.js}, scris în JavaScript pur. Alegerea de a nu folosi TypeScript aici a fost deliberată. Codul rămâne la nivelul unui apel \texttt{fetch} standard, ușor de urmărit de oricine cunoaște API-ul \texttt{fetch} al browserelor.
...
\end{itemize}
Constrângerile semantice (un folder are între 2 și 4 dispozitive, fiecare \texttt{deviceId} apare cel mult o dată) sunt verificate atât pe client cât și pe backend.
...
Modulul fotovoltaic dispune de o pagină dedicată la \texttt{/dashboard/solar/}, accesibilă tuturor rolurilor autentificate. Aceasta consumă API-ul descris în Capitolul 2 (în special \texttt{GET /api/solar/systems/<id>/overview/}) și este alcătuită din componente specializate, izolate sub \texttt{components/solar/}.
...
% FIGURĂ COMPUSĂ 2x2: fluxul de onboarding. Fiecare panou afișează automat
...
% onboarding_complete.png, onboarding_checklist.png.
