...
.3 notifications/.
.3 onboarding/.
.3 ui/\DTcomment{28 primitive shadcn/ui}.
...
\texttt{PowerFlowDiagram.tsx} construiește o reprezentare animată SVG, inspirată de interfețele Fronius SolarWeb și Tesla. Patru noduri periferice (PV, rețea, consum casnic, baterie) sunt distribuite în jurul unui nod central reprezentând invertorul. Direcțiile fluxurilor sunt deduse din semnele câmpurilor \texttt{power.solarW}, \texttt{power.gridW}, \texttt{power.loadW} și \texttt{power.batteryW} prin funcția \texttt{deriveFlows} din \texttt{components/solar/solar-utils.ts}; nodul de baterie este afișat doar atunci când câmpul \texttt{battery.present} este adevărat. Pragul de inactivitate este de 1 W per latură, valori sub acest prag determinând estomparea nodurilor corespunzătoare și o animație discretă de tip „respirație" (\texttt{solar-idle-breathe}) care comunică starea de repaus fără a sugera mișcare. Toate dimensiunile sunt parametrizate prin proprietățile CSS \texttt{NODE\_R} și \texttt{HUB\_R}, permițând adaptarea automată pentru ecrane mici.
...
Pentru ca interfața să reflecte fidel ce widget-uri sunt cu adevărat procesate end-to-end (firmware $\rightarrow$ MQTT $\rightarrow$ backend $\rightarrow$ card), paleta este alimentată de endpoint-ul \texttt{GET /api/widget-types/} descris în Capitolul 2. Tipurile \emph{supported} (în prezent \texttt{TOGGLE}, \texttt{SLIDER}, \texttt{TEMPERATURE}, \texttt{HUMIDITY}, \texttt{PRESSURE}) pot fi plasate liber pe canvas, în timp ce tipurile \emph{future} (\texttt{MOTION}, \texttt{LIGHT}, \texttt{CO2}) sunt randate ca plăci estompate, marcate cu un badge „Soon" și inactive la \emph{drag-and-drop} sau \emph{click}, generând un mesaj \texttt{toast} explicativ. Aceeași logică se aplică și în paleta de componente (microcontroler, senzori, releu) din panoul de topologie: doar tipurile cu suport firmware complet sunt selectabile. Senzorul de presiune (BMP180) a fost recent promovat din \emph{future} în \emph{supported}, odată cu integrarea citirilor de presiune barometrică în firmware-ul ESP32.
